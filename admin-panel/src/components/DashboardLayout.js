...
import { Box, Drawer, AppBar, Toolbar, Typography, IconButton, Avatar, TextField, InputAdornment, Badge, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, Button, } from '@mui/material';
import { Menu as MenuIcon, Dashboard as DashboardIcon, Article as ArticleIcon, RssFeed as BlogIcon, Psychology as KnowledgeIcon, ContactMail as ContactIcon, School as AlumniIcon, Search as SearchIcon, Notifications as NotificationsIcon, Settings as SettingsIcon, Logout as LogoutIcon, } from '@mui/icons-material';
...
function SidebarContent({ onNavigate }) {
    ...
    return (<Box sx={{ bgcolor: SIDEBAR_BG, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid rgba(0,0,0,0.08)' }}>
      {/* ── Logo ─────────────────────────────────────────────── */}
        ...
        {menuGroups.map((group) => {
            ...
            return (<Box key={group.label}>
              ...
              {visibleItems.map((item) => {
                    ...
                    return (<Box key={item.text} onClick={() => onNavigate(item.path)} sx={{
      ...
      <Box sx={{
        ...
        <Tooltip title="Logout">
          <IconButton size="small" onClick={handleLogoutClick} sx={{ color: SIDEBAR_TEXT, '&:hover': { color: '#ef4444', bgcolor: 'rgba(239,68,68,0.08)' } }}>
      ...
      <Dialog open={showLogoutModal} onClose={handleLogoutCancel} maxWidth="xs" fullWidth PaperProps={{
            sx: {
                borderRadius: 3,
        ...
        <DialogActions sx={{
          ...
          <Button onClick={handleLogoutCancel} variant="outlined" sx={{
            minWidth: 100,
            py: 1.2,
            borderRadius: 2,
            borderColor: '#e2e8f0',
...
export default function DashboardLayout() {
    ...
    return (<Box sx={{ display: 'flex', bgcolor: '#f4f5fa', minHeight: '100vh' }}>
      ...
      <AppBar position="fixed" elevation={0} sx={{
        ...
        <Toolbar sx={{ gap: 1.5, minHeight: '60px !important' }}>
          <IconButton edge="start" onClick={() => setMobileOpen(true)} sx={{ display: { sm: 'none' }, color: '#2d3748' }}>
            <MenuIcon />
          </IconButton>

          {/* Search */}
          <TextField size="small" placeholder="Search (Ctrl+/)" sx={{
            maxWidth: 260,
      ...
      <Box component="nav" sx={{ width: { sm: DRAWER_WIDTH }, flexShrink: { sm: 0 } }}>
        {/* Mobile */}
        <Drawer variant="temporary" open={mobileOpen} onClose={() => setMobileOpen(false)} ModalProps={{ keepMounted: true }} sx={{ display: { xs: 'block', sm: 'none' }, '& .MuiDrawer-paper': { width: DRAWER_WIDTH, bgcolor: SIDEBAR_BG, border: 'none' } }}>
          <SidebarContent onNavigate={handleNavigate}/>
        </Drawer>
        {/* Desktop */}
        <Drawer variant="permanent" open sx={{ display: { xs: 'none', sm: 'block' }, '& .MuiDrawer-paper': { width: DRAWER_WIDTH, bgcolor: SIDEBAR_BG, border: 'none' } }}>
          <SidebarContent onNavigate={handleNavigate}/>
        </Drawer>
      </Box>
