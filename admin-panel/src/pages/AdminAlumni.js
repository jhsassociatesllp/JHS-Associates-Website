...
const AdminAlumni = () => {
    ...
    const filteredAlumni = alumni.filter(person => person.first_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        person.last_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        person.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
        person.company.toLowerCase().includes(searchTerm.toLowerCase()) ||
        person.designation.toLowerCase().includes(searchTerm.toLowerCase()));
    // Pagination
    const paginatedAlumni = filteredAlumni.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
    const totalPages = Math.ceil(filteredAlumni.length / rowsPerPage);
    return (<div className="alumni-container">
      {/* Header */}
      ...
      <div className="alumni-action-bar">
        ...
        <div className="alumni-actions">
          <button className="alumni-btn alumni-btn-icon icon-refresh" onClick={fetchAlumni} title="Refresh">
          </button>
      ...
      <div className="alumni-main-card">
        ...
        {loading ? (<div className="alumni-loading">
            <div className="alumni-spinner"></div>
          </div>) : (<>
            {/* Cards Grid */}
            <div className="alumni-cards-grid">
              {paginatedAlumni.map((person) => (<div key={person.id} className="alumni-card">
                  ...
                  <div className="alumni-card-body">
                    ...
                    {person.phone && (<div className="alumni-detail">
                        <span className="detail-label">Phone:</span>
                        <span className="detail-value">{person.phone}</span>
                      </div>)}

                    {person.message && (<div className="alumni-message">
                  ...
                  <div className="alumni-card-footer">
                    ...
                    <div className="alumni-actions">
                      <button className="alumni-action-btn reply" onClick={() => window.open(`mailto:${person.email}`, '_blank')} title="Send Email">
                        ✉
                      </button>
                      {person.phone && (<button className="alumni-action-btn call" onClick={() => window.open(`tel:${person.phone}`, '_blank')} title="Call">
                          📞
                        </button>)}
                    </div>
                  </div>
            ...
            {/* Empty State */}
            {filteredAlumni.length === 0 && !loading && (<div className="alumni-empty">
            ...
            {/* Pagination */}
            {filteredAlumni.length > 0 && (<div className="alumni-pagination">
                <div>
