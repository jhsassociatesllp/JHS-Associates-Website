...
const AdminKnowledge = () => {
    ...
    const handleDelete = async (id) => {
        if (!confirm('Are you sure you want to delete this knowledge resource?'))
            return;
        try {
    ...
    const filteredResources = knowledgeResources.filter(resource => resource.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (<div className="knowledge-container">
      ...
      <div className="knowledge-action-bar">
        ...
        <div className="knowledge-actions">
          <button className="knowledge-btn knowledge-btn-icon icon-refresh" onClick={fetchKnowledgeResources} title="Refresh">
          </button>
          
          <button className="knowledge-btn knowledge-btn-primary" onClick={() => {
            resetForm();
            setShowForm(true);
        }}>
      ...
      <div className="knowledge-main-card">
        ...
        {loading ? (<div className="knowledge-loading">
            ...
            <div className="knowledge-table-container">
              <table className="knowledge-table">
                ...
                <tbody>
                  {paginatedResources.map((resource) => (<tr key={resource.id}>
                      <td>
                      ...
                      <td>
                        <span className={`knowledge-status-chip ${resource.last_edited_at ? 'knowledge-status-updated' : 'knowledge-status-published'}`}>
                          {resource.last_edited_at ? 'Updated' : 'Published'}
                        </span>
                      </td>
                      <td>
                        <div className="knowledge-table-actions">
                          <button className="knowledge-action-btn knowledge-action-edit" onClick={() => handleEdit(resource)} title="Edit">
                          </button>
                          <button className="knowledge-action-btn knowledge-action-delete" onClick={() => handleDelete(resource.id)} title="Delete">
                          </button>
                          <a href={`${API_BASE}/knowledge/pdf/${resource.pdf_id}`} target="_blank" rel="noopener noreferrer" className="knowledge-action-btn knowledge-action-download" title="Download PDF">
                          </a>
                        </div>
                      </td>
                    </tr>))}
      ...
      {showForm && (<div className="knowledge-dialog-backdrop" onClick={() => setShowForm(false)}>
          <div className="knowledge-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="knowledge-form-header">
              ...
              <button className="knowledge-close-btn" onClick={() => setShowForm(false)} aria-label="Close">
                ×
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="knowledge-form-container">
                  ...
                  <div className="knowledge-form-section">
                    ...
                    <div className="knowledge-form-field">
                      ...
                      <input type="text" className="knowledge-form-input" placeholder="Enter a compelling title that captures the essence of your resource..." value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} required/>
                    </div>
                    ...
                    <div className="knowledge-form-field">
                      ...
                      <textarea className="knowledge-form-input knowledge-form-textarea" rows={3} placeholder="Write a brief, compelling summary that will appear on resource cards and attract users to learn more..." value={formData.short_description} onChange={(e) => setFormData(prev => ({ ...prev, short_description: e.target.value }))} required/>
                    ...
                    <div className="knowledge-form-field">
                      ...
                      <textarea className="knowledge-form-input knowledge-form-textarea" rows={8} placeholder="Provide comprehensive, valuable content that users will see when they open the resource modal. Include detailed information, insights, and actionable guidance..." value={formData.content} onChange={(e) => setFormData(prev => ({ ...prev, content: e.target.value }))} required/>
                      <p className="knowledge-form-helper">
                  ...
                  <div className="knowledge-form-section">
                    ...
                    <div className="knowledge-upload-grid">
                      ...
                      <div className="knowledge-form-field">
                        ...
                        <div className="knowledge-upload-zone" onClick={() => document.getElementById('image-upload')?.click()}>
                          <input type="file" accept="image/*" onChange={handleImageChange} style={{ display: 'none' }} id="image-upload" required={!editingId}/>
                  ...
                  Max size: 10MB
                      ...
                      <div className="knowledge-form-field">
                        ...
                        <div className="knowledge-upload-zone pdf" onClick={() => document.getElementById('pdf-upload')?.click()}>
                          <input type="file" accept=".pdf" onChange={handlePdfChange} style={{ display: 'none' }} id="pdf-upload" required={!editingId}/>
                    ...
                    {!editingId && (<div className="knowledge-upload-required">
              ...
              <div className="knowledge-form-footer">
                ...
                <div className="knowledge-form-footer-actions">
                  <button type="button" className="knowledge-form-btn knowledge-form-btn-cancel" onClick={() => setShowForm(false)}>
                    Cancel
                  </button>
                  
                  <button type="submit" className="knowledge-form-btn knowledge-form-btn-submit" disabled={submitting}>
                    {submitting ? (<div className="knowledge-form-btn-loading">
                        <div className="knowledge-form-spinner"></div>
                        <span>Saving...</span>
                      </div>) : (<span>{editingId ? 'Update Resource' : 'Create Resource'}</span>)}
                  </button>
                </div>
              </div>
            </form>
