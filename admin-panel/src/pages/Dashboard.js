...
import { Typography, Box, Card, CardContent, Chip, ToggleButtonGroup, ToggleButton, CircularProgress, } from '@mui/material';
import { Article as ArticleIcon, Description as BlogIcon, Email as EmailIcon, Assessment as AssessmentIcon, MenuBook as KnowledgeIcon, } from '@mui/icons-material';
import { AreaChart, Area, BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, PieChart, Pie, } from 'recharts';
...
export default function Dashboard() {
    ...
    const generateMonthlyData = (articles, blogs, knowledge, contacts) => {
    ...
    const generateContentDistribution = (articlesCount, blogsCount, knowledgeCount) => {
    ...
    const generateRecentActivity = (articles, blogs, knowledge, contacts, alumni) => {
        const activities = [];
        // Add recent articles
    ...
    if (loading) {
        return (<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
    ...
    return (<Box sx={{ p: { xs: 2, md: 3 }, bgcolor: C.pageBg, minHeight: '100%' }}>
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
          ...
          <Chip label="Last updated: Just now" size="small" sx={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        {STAT_CARDS.map((card) => {
            ...
            return (<Card key={card.label} elevation={0} sx={{
                    bgcolor: C.cardBg,
                ...
                <CardContent sx={{ p: '20px !important' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 2 }}>
                    ...
                    <Chip label={card.change} size="small" sx={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px', marginBottom: '24px' }}>
        ...
        <Card elevation={0} sx={{ bgcolor: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 2, height: '100%' }}>
          <CardContent sx={{ p: '20px !important' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
              ...
              <ToggleButtonGroup size="small" value={timeRange} exclusive onChange={(_, v) => v && setTimeRange(v)} sx={{
        ...
        ].map((item) => (<Box key={item.label} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Card elevation={0} sx={{ bgcolor: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 2, height: '100%' }}>
          <CardContent sx={{ p: '20px !important' }}>
            ...
            <Box sx={{ height: 240, position: 'relative' }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={contentDistribution} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={3} dataKey="value" startAngle={90} endAngle={-270}>
                    {contentDistribution.map((entry, i) => (<g key={i}>
            ...
            {/* Legend */}
            <Box sx={{ mt: 2 }}>
              {contentDistribution.map((item) => (<Box key={item.name} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
        ...
        <Card elevation={0} sx={{ bgcolor: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 2 }}>
          <CardContent sx={{ p: '20px !important' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {recentActivity.length > 0 ? recentActivity.map((activity) => (<Box key={activity.id} sx={{
                display: 'flex',
                alignItems: 'center',
                gap: 1.5,
                p: 1.5,
                ...
                flexShrink: 0,
                    ...
                    {activity.type === 'article' ? (<ArticleIcon sx={{ fontSize: 18, color: C.accent }}/>) : activity.type === 'blog' ? (<BlogIcon sx={{ fontSize: 18, color: C.success }}/>) : activity.type === 'knowledge' ? (<KnowledgeIcon sx={{ fontSize: 18, color: C.info }}/>) : (<EmailIcon sx={{ fontSize: 18, color: C.warning }}/>)}
                  </Box>
                ...
                whiteSpace: 'nowrap',
                  ...
                  <Chip label={activity.status} size="small" sx={{
                bgcolor: activity.status === 'published' ? `${C.success}15` :
                    activity.status === 'updated' ? `${C.info}15` :
                        activity.status === 'resolved' ? `${C.success}15` : `${C.warning}15`,
                color: activity.status === 'published' ? C.success :
                ...
                </Box>)) : (<Box sx={{ textAlign: 'center', py: 4 }}>
        ...
        <Card elevation={0} sx={{ bgcolor: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 2 }}>
          <CardContent sx={{ p: '20px !important' }}>
            ...
            boxShadow: C.shadow,
                  ...
                  <Area type="monotone" dataKey="value" stroke={C.accent} strokeWidth={3} fill="url(#weeklyGrad)" dot={{ r: 4, fill: C.accent, strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 6, fill: C.accent }}/>
                </AreaChart>
              </ResponsiveContainer>
            </Box>
          </CardContent>
