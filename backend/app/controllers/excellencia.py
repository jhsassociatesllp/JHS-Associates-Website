...
async def create_excellencia(data: ExcellenciaCreate) -> dict:
    db = get_database()
...
async def update_excellencia(excellencia_id: str, data: ExcellenciaUpdate) -> dict:
    db = get_database()
    collection = db[COLLECTION]

    current = await collection.find_one({"_id": ObjectId(excellencia_id)})
    if not current:
        raise ValueError("Excellencia entry not found")
    ...
    updated = await collection.find_one({"_id": ObjectId(excellencia_id)})
    return _serialize(updated)


async def get_all_excellencia() -> list[dict]:
    db = get_database()
    collection = db[COLLECTION]
    entries = []
...
async def get_excellencia_by_id(excellencia_id: str) -> dict | None:
    db = get_database()
    collection = db[COLLECTION]
    doc = await collection.find_one({"_id": ObjectId(excellencia_id)})
    return _serialize(doc) if doc else None


async def delete_excellencia(excellencia_id: str) -> bool:
    db = get_database()
    collection = db[COLLECTION]

    entry = await collection.find_one({"_id": ObjectId(excellencia_id)})
    if entry and entry.get("image_id"):
        fs = AsyncIOMotorGridFSBucket(db)
        try:
            await fs.delete(ObjectId(entry["image_id"]))
        except Exception:
            pass

    result = await collection.delete_one({"_id": ObjectId(excellencia_id)})
