...
from app.schemas.regulatory import RegulatoryCreate, RegulatoryUpdate
...
async def create_regulatory(data: RegulatoryCreate) -> dict:
    db = get_database()
...
async def update_regulatory(regulatory_id: str, data: RegulatoryUpdate) -> dict:
    db = get_database()
    collection = db[COLLECTION]

    current = await collection.find_one({"_id": ObjectId(regulatory_id)})
    if not current:
        raise ValueError("Regulatory not found")
    ...
    updated = await collection.find_one({"_id": ObjectId(regulatory_id)})
    return _serialize(updated)


async def get_all_regulatory() -> list[dict]:
    db = get_database()
    collection = db[COLLECTION]
    items = []
...
async def get_regulatory_by_id(regulatory_id: str) -> dict | None:
    db = get_database()
    collection = db[COLLECTION]
    doc = await collection.find_one({"_id": ObjectId(regulatory_id)})
...
async def delete_regulatory(regulatory_id: str) -> bool:
    ...
    regulatory = await collection.find_one({"_id": ObjectId(regulatory_id)})
    if regulatory:
        fs = AsyncIOMotorGridFSBucket(db)
        for field in ("pdf_id", "image_id"):
            if regulatory.get(field):
                try:
                    await fs.delete(ObjectId(regulatory[field]))
                except Exception:
                    pass

    result = await collection.delete_one({"_id": ObjectId(regulatory_id)})
