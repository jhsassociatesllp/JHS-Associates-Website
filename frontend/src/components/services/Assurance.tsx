...
export default function Assurance() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<Element>('.aa-section-header').forEach((header) => {
        gsap.fromTo(header,
          { opacity: 0, y: 30 },
      ...
      gsap.utils.toArray<Element>('.aa-spoke-row--left').forEach((row, i) => {
        gsap.fromTo(row,
          { opacity: 0, x: -50 },
      ...
      gsap.utils.toArray<Element>('.aa-spoke-row--right').forEach((row, i) => {
        gsap.fromTo(row,
          { opacity: 0, x: 50 },
          { opacity: 1, x: 0, duration: 0.6, delay: i * 0.08, scrollTrigger: { trigger: '.aa-spoke-diagram', start: 'top 75%' } }
        )
