...
export default function Taxation() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray<Element>('.tx-section-header').forEach((header) => {
        gsap.fromTo(header,
          { opacity: 0, y: 30 },
      ...
      gsap.utils.toArray<Element>('.tx-spoke-row--left').forEach((row, i) => {
        gsap.fromTo(row,
          { opacity: 0, x: -50 },
      ...
      gsap.utils.toArray<Element>('.tx-spoke-row--right').forEach((row, i) => {
        gsap.fromTo(row,
          { opacity: 0, x: 50 },
          { opacity: 1, x: 0, duration: 0.6, delay: i * 0.08, scrollTrigger: { trigger: '.tx-spoke-diagram', start: 'top 75%' } }
        )
