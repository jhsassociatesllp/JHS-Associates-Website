...
file_path = r'f:\Maaz\JHS-Associates-Website\frontend\src\components\Insights\Articles.css'
...
css = re.sub(
    r'\.art-hero__title em\s*\{[^}]+\}',
    r'.art-hero__title em {\n  font-style: normal;\n}',
    css
)

css = re.sub(
    r'\.art-hero__eyebrow\s*\{[^}]+\}',
...
# 5. Fix learn btn background
css = css.replace('background: #3765b0;', 'background: linear-gradient(135deg, var(--red), var(--red-hi));')

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(css)

print('Updated Articles.css via Python')
